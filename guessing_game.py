...
def get_word():
    k = randrange(len(word_rus))
    return word_rus[k].upper()
...
def play(word):
    ...
    print('Давайте играть в угадайку слов! Я загадаю слово на русском языке, а вы попробуете его угадать! У вас всего'
          ' 6 попыток! Поехали!')
    print(f'Сейчас вы на этой стадии {display_hangman(tries)}')
    print(f'Загаданное слово выглядит так: {word_completion}. Количество букв: {len(word)}')
    while tries != 0:
        guessed = True
        trie = input('Введите букву, которая может быть в слове, или слово целиком, если вы его уже угадали: ').upper()
        while True:
            if not trie.isalpha():
                print('Вы ввели некорректный символ, вводите только буквы')
                guessed = False
                break
            elif trie.isalpha():
                for i in trie:
                    if i in eng:
                        print('Используй только русский алфавит!')
                        guessed = False
                        break
                if not guessed:
                    break
                if 1 < len(trie) < len(word):
                    print('Вы ввели больше одной буквы, или меньше букв, чем в слове. Повторю: введите одну букву, или '
                          'слово целиком')
                    guessed = False
                    break
                elif trie in guessed_words or trie in guessed_letters:
                    print('Повтор, вы уже пробовали этот вариант!')
                    guessed = False
                    break
                elif len(trie) > len(word):
                    print('Ваше слово длиннее загаданного! Пока не засчитываю это как ошибку, но будь внимательей!')
                    guessed = False
                    break
                break
            break
        ...
        if trie == word:
            ...
            guessed_letters.append(trie)
            for i in range(len(word)):
                if trie == word[i]:
                    word_completion = word_completion[:i] + trie + word_completion[i + 1:]
            print(f'Поздравляем, вы угадали букву! Приоткроем слово: {word_completion}')
            ...
            if len(trie) == 1:
                ...
                guessed_words.append(trie)
            tries -= 1
            if tries > 1:
                print(f'Вы не угадали. Пробуйте снова. Количество попыток: {tries}')
                print(display_hangman(tries))
            elif tries == 1:
                print('Последняя попытка! Человечек на волоске!!!')
                print(display_hangman(tries))
    if tries == 0:
        print(f'К сожалению вы проиграли. Я загадал слово {word}')
...
while True:
    ...
    play(word)
    while True:
        again = input('Сыграем ещё? Да или Нет: ').upper()
        yes = ['ДА', 'Д', 'ДАА', 'ДАВАЙ', 'КОНЕЧНО', 'YES', 'Y', 'OF COURSE', 'SURE' ]
        no = ['НЕТ', 'Н', 'НИЗАЧТО', 'КОНЕЧНО НЕТ', 'НЕЕЕТ', 'НЕ', 'НЕА', 'NO', 'N', 'NOO', 'NEVER']
        if again in yes:
            flag = True
            break
        elif again in no:
            ...
            print('Не понял тебя, выражайся яснее.')
            continue
    if not flag:
        print('До свидания!')
        break